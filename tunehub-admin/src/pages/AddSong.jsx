...
const AddSong = () => {
    ...
    const loadAlbumData = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                toast.error("Unable to get album data");
            }
        } catch (err) {
            toast.error("Error occurred");
        }
    ...
    useEffect(() => {
        loadAlbumData();
    }, []); // Corrected to run only once when the component mounts
            ...
            <div className='flex flex-col gap-2.5'>
                ...
                <select onChange={(e) => setAlbum(e.target.value)} value={album} className='bg-transparent outline-green-600 border-2 border-gray-400 p-2.5 w-[150px]'>
                    ...
                    {albumData.map((item, index) => (
                        <option key={index} value={item.name}>{item.name}</option> // Changed to use _id for album selection
                    ))}
                </select>
            </div>
